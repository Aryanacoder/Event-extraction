...
def continuous_severity_classification(deaths, injured, affected, homeless, alpha=0.5, beta=0.2, gamma=0.15, delta=0.15):
...
# Log cluster severity scores to TensorBoard
with file_writer.as_default():
    for i in range(num_clusters):
        tf.summary.scalar(f'Cluster_{i}_Severity_Score', cluster_severity_scores[i], step=1)
...
# Log PCA visualization to TensorBoard
...
with file_writer.as_default():
    tf.summary.image("K-Means Clustering", image, step=1)
